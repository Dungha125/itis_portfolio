...
import { slideInFromBottomtime } from '@/ulti/motion';
import { useInView } from 'react-intersection-observer';
var ActivitySpecialContent = function () {
    var _a = useInView({
        triggerOnce: true,
        threshold: 0.15,
    ...
    return (<motion.div id='special' ref={ref} initial="hidden" animate={inView ? 'visible' : 'hidden'} variants={containerVariants} className='flex flex-col items-center justify-center gap-4 w-full bg-white z-[30] pb-[4rem]'>
    ...
    {ActivitySpecialText.map(function (item) { return (<motion.div variants={slideInFromBottomtime(0.6 + parseInt(item.id) * 0.1)} key={item.id} className='w-full flex flex-col lg:flex-row rounded-2xl shadow-lg bg-info-100 h-auto lg:h-[20rem] relative overflow-hidden'>
            <div className='w-full lg:w-1/2 h-[15rem] lg:h-full overflow-hidden rounded-t-lg lg:rounded-l-lg lg:rounded-tr-none'>
                <img src={item.sour} alt={item.title} className='object-cover w-full h-full rounded-t-lg lg:rounded-l-lg lg:rounded-tr-none'/>
            </div>
