...
var ActivityAnualContent = function () {
    var _a = useInView({
        triggerOnce: true,
        threshold: 0.15,
    ...
    return (<div className="flex flex-col items-center justify-center gap-4 w-full bg-white z-[30] pb-[4rem]">
    <motion.div id='annual' ref={ref} initial="hidden" animate={inView ? 'visible' : 'hidden'} variants={containerVariants} className="flex flex-col items-center justify-center gap-4 w-full bg-white z-[30] ">
        ...
        <div className='flex flex-wrap items-center justify-center  px-10 lg:px-20 gap-4 lg:pt-8'>
          {activityannual.map(function (item) { return (<motion.div variants={slideInFromLeft(0.6 + 0.1 * parseInt(item.id))} className="block relative  bg-no-repeat bg-[length:100%_100%] min-w-[20rem]" key={item.id} style={{
            ...
            <div className="absolute inset-0.5 bg-n-8 -z-10 " style={{ clipPath: "url(#benefits)" }}>
                  ...
                  <div className="absolute inset-0">
                    {item.imageUrl && (<img src={item.imageUrl} width={900} height={900} className="w-full h-full object-cover"/>)}
