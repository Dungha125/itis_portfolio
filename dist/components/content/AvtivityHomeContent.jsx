...
var AvtivityHomeContent = function () {
    var _a = useInView({
        triggerOnce: true,
        threshold: 0.15,
    ...
    return (<motion.div ref={ref} initial="hidden" animate={inView ? 'visible' : 'hidden'} variants={containerVariants} className="flex flex-col items-center justify-center gap-4 w-full z-[30]">
        ...
        <div className='flex flex-wrap items-center justify-center  px-10 lg:px-20 gap-4 mt-4 lg:pt-8'>
          {activity.map(function (item) { return (<motion.div variants={slideInFromLeft(0.6 + 0.1 * parseInt(item.id))} className="block relative  bg-no-repeat bg-[length:100%_100%] min-w-[20rem]" key={item.id} style={{
