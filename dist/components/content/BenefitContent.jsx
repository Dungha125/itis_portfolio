...
var BenefitContent = function () {
    ...
    <motion.div ref={ref} initial="hidden" animate={inView ? 'visible' : 'hidden'} variants={containerVariants} className="flex flex-col items-center justify-center gap-4 w-full z-[30]">
        ...
        <div className='flex flex-wrap items-center justify-center  px-10 lg:px-20 gap-4 mt-4'>
          {benefittext.map(function (item) { return (<motion.div variants={slideInFromLeft(0.6 + 0.1 * parseInt(item.id))} className="block relative p-0.5 bg-no-repeat bg-[length:100%_100%] md:max-w-[24rem]" key={item.id} style={{
